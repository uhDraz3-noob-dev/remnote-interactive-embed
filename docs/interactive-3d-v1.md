...
Host integration (fixed in 0.3.1): derive the packaged engine URL from the SDK's `plugin.rootURL` and pass it explicitly to `makeDocument`. Do not derive it from `window.location`, `import.meta.url`, or author HTML. The loader currently supports HTTP(S) asset roots; missing/unsupported roots retain the 2D fallback. The SDK sets the root before mounting React. `scripts/test-asset-loading.cjs` runs the real widget with an SDK test host whose page and asset roots differ, so a manually supplied test-only engine URL cannot hide this regression.
